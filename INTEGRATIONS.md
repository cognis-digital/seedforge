...
**seedforge** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `seedforge` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
seedforge ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
seedforge ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
seedforge ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `seedforge` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="seedforge") for rec in records]   # records = your JSON output
